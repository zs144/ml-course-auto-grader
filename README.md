...
- The `P_d` and `P_fa` column in the ROC data table is not sorted.
  ...
  This will cause the ROC curve to bounce back and forth, forming a messy graph. Pre-checking if the ROC data input is monotonic increasing/decreasing. Use `diff()` to catch this error.
